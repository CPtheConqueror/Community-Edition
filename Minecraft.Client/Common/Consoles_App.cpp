...
#ifdef _WINDOWS64
unordered_map<wstring, DLC_INFO*> CMinecraftApp::DLCInfo_W64;
#endif

#if _WINDOWS64
void CMinecraftApp::RegisterW64DLC(eDLCContentType eType,
                                   const wchar_t*  pwchProductID,
                                   const wchar_t*  pwchBannerTextureName,
                                   int             iConfig,
                                   unsigned int    uiSortIndex,
                                   PBYTE           pbBannerData,
                                   DWORD           dwBannerBytes)
{
    if (pwchProductID == nullptr)
        return;

    wstring key(pwchProductID);

    // If an entry already exists for this product ID, free it before replacing
    auto it = DLCInfo_W64.find(key);
    if (it != DLCInfo_W64.end())
    {
        delete it->second;
        DLCInfo_W64.erase(it);
    }

    DLC_INFO *pInfo = new DLC_INFO();
    memset(pInfo, 0, sizeof(DLC_INFO));

    pInfo->eDLCType    = eType;
    pInfo->uiSortIndex = uiSortIndex;
    pInfo->pbImageData = pbBannerData;
    pInfo->dwImageBytes = dwBannerBytes;

    // Store the banner texture name/key
    if (pwchBannerTextureName != nullptr)
        wcsncpy(pInfo->wchBanner, pwchBannerTextureName, ARRAYSIZE(pInfo->wchBanner) - 1);

    DLCInfo_W64[key] = pInfo;
}
#endif
...
#ifdef _WINDOWS64
DLC_INFO* CMinecraftApp::GetDLCInfoByProductID(const wchar_t* pwchProductID)
{
    wstring key(pwchProductID);
    auto it = DLCInfo_W64.find(key);
    if (it == DLCInfo_W64.end())
