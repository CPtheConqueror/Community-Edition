...
UIScene_MainMenu::UIScene_MainMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	if(ProfileManager.IsFullVersion())
	{
		...
		m_buttons[(int)eControl_UnlockOrDLC].init(L"LCE Store",eControl_UnlockOrDLC);
	}
	else
	{
...
void UIScene_MainMenu::handleGainFocus(bool navBack)
{
	...
	if(navBack && ProfileManager.IsFullVersion())
	{
		...
		m_buttons[(int)eControl_UnlockOrDLC].setLabel(L"LCE Store");
	}
...
// 			ui.RequestMessageBox(IDS_PATCH_AVAILABLE_TITLE, IDS_PATCH_AVAILABLE_TEXT, uiIDA, 1, XUSER_INDEX_ANY,NULL,this);
		...
		if(ProfileManager.IsSignedInLive(iPad))
		{
			...
			else
			{
				...
				{
					// Changing to async TMS calls (on Windows64 this uses localhost store server via Windows64_StorageHttp)
...
void UIScene_MainMenu::tick()
{
	...
	{	
#if defined(_WINDOWS64)
		// On Windows64 with Windows64_App there is no TMS callback; skip wait so we don't hang. With localhost store (Windows64_StorageHttp) a Durango-style build would complete via HTTP.
		m_bWaitingForDLCInfo=false;
